...
// Demo build: a ready-to-send email per client, so the player only has to
// press send. Fields stay editable in case anyone wants to tweak them.
const CLIENT_EMAIL_DRAFTS: Record<string, { subject: string; body: string }> = {
  'test-level-1': {
    subject: "Quick chat about ACMD's supply chain visibility",
    body: `Hi Sarah,

I understand ACMD Manufacturing has been dealing with supply-chain delays, partly because inventory, order and logistics information sits in separate systems, which makes it hard to catch problems early.

I'd like to discuss how we could help improve visibility across your supply chain without a disruptive replacement of the systems you already rely on, so you can spot issues sooner and reduce delivery delays as ACMD continues to grow.
...
IBM Consulting`,
  ...
  'test-level-2': {
    subject: 'A reliable customer view for Meridian Retail',
    body: `Hi David,

I understand Meridian Retail's customer data is spread across stores, online, mobile and loyalty systems, making it difficult for your teams to work from one reliable view of the customer as you prepare to expand.

I'd like to discuss a practical solution that would help bring that data together and give your business teams a consistent view they can act on quickly, without duplicating the strong work your internal technology team already does.

Would you have time for a short call next week to explore this?
...
export function createOutreachLaptopFlow(
  ...
  const shell = (content: string, action = '', laptopAction = '', screenAction = ''): string => `
    <style>
      .l2-shell{width:1320px;height:620px;display:grid;grid-template-columns:830px 450px;align-items:start;gap:40px;background:transparent;font-family:Arial,sans-serif;color:#161616;box-sizing:border-box}
      ...
      /* iPad/touch: without this, a tap can register as the browser's default
         double-tap-to-zoom gesture instead of an immediate click, so buttons and
         links feel unresponsive on first touch. */
      .l2-shell button,.l2-shell input,.l2-shell textarea,.l2-shell [data-client],.l2-shell .l2-link,.l2-shell .l2-result{touch-action:manipulation}
      ...
      .l2-task.active{color:#001d6c}.l2-task.active span{border-color:#001d6c;background:#d9effa}.l2-task.done{color:#002d9c}.l2-task.done span{border-color:#002d9c;background:#d0e2ff}
      .l2-action{margin-top:auto;min-height:70px}.l2-button{border:3px solid #161616;border-radius:10px;background:#002d9c;color:#fff;padding:13px 20px;font-size:17px;font-weight:700;box-shadow:5px 6px 0 #161616;cursor:pointer}.l2-button:hover{transform:translateY(-2px)}.l2-button:disabled{border-color:#c8b998;background:#edf5ff;color:#96999c;box-shadow:none;cursor:not-allowed}
      ...
      .l2-sent{display:grid;height:100%;place-items:center;text-align:center}.l2-sent-mark{font-size:86px;color:#002d9c}.l2-sent h2{font-size:32px;color:#001d6c}.l2-sent p{max-width:500px;font-size:18px;line-height:1.5}
  ...
  const render = (): void => {
    ...
    if (step === 'clients') {
      ...
      root.innerHTML = shell(
        ...
        `<button class="l2-button" data-action="search-go">Search</button>`
      )
      bind('[data-action="search"]', 'click', () => {
        step = 'results'
        render()
      })
      bind('[data-action="search-go"]', 'click', () => {
        step = 'results'
        render()
      })
