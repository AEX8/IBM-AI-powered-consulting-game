...
export function applyStageCompletion(
  ...
  const xpForThisRun = xpForStage(input.stageId, input.performance)
  const previousXp = previous?.xp ?? 0
  const xpAwarded = Math.max(0, xpForThisRun - previousXp)

  const shouldAwardSkills = !previous?.skillsAwarded && input.skillDeltas !== undefined
  ...
  const bestPerformance: Performance =
    previous && previous.xp > xpForThisRun ? previous.performance : input.performance
  ...
  return {
    data: {
      ...
      skillStats,
      completedLevels: current.completedLevels.includes(input.stageId)
        ? current.completedLevels
        : [...current.completedLevels, input.stageId].sort((a, b) => a - b),
      badges: current.badges.includes(badgeId) ? current.badges : [...current.badges, badgeId],
      stageResults: {
...
// Client keys (e.g. 'sarah') the player has completed a given stage with.
export function completedClientKeys(data: ProgressData, stageId: number): string[] {
  const prefix = `${stageId}_`

  return Object.keys(data.stageResults)
